...
def initialize():
    ...
    #with open("temperature_stats.csv", "a", newline='') as f:
        #writer = csv.writer(f)
        #writer.writerow(
            #["Timestamp", "High Today", " Time  ", "Low Today", "  Time ", "High Year", " date ", "Low Year", " date "])
...
def update_stats(temp2):
    ...
    if hi_today is None or temp2 > hi_today:
        hi_today = temp2
        hi_time = datetime.datetime.now().strftime('%H:%M')
    if lo_today is None or temp2 < lo_today:
        lo_today = temp2
        lo_time = datetime.datetime.now().strftime('%H:%M')

    if hi_year is None or temp2 > hi_year:
        hi_year = temp2
        hi_date = datetime.datetime.now().strftime('%H:%M')
    if lo_year is None or temp2 < lo_year:
        lo_year = temp2
        lo_date = datetime.datetime.now().strftime('%H:%M')


def reset_daily_stats():
    global hi_today, lo_today
    stats_hiyes.setText(hi_today)
    stats_loyes.setText(lo_today)
    hi_today = None
    lo_today = None
...
def display_stats():
    stats_hiday.setText(str(hi_today) + " : " + hi_time)
    stats_loday.setText(str(lo_today) + " : " + lo_time)
    # stats_hiyes.setText(hi_yes)
    # stats_loyes.setText(lo_yes)
    stats_hiyear.setText(str(hi_year) + " : " + hi_date)
    stats_loyear.setText(str(lo_year) + " : " + lo_date)
...
def append_to_file():
    ...
    with open("temperature_stats.csv", "a", newline='') as f:
        writer = csv.writer(f)
        writer.writerow(
            [timestamp, hi_today, hi_time, lo_today, lo_time, hi_year, hi_date,
             lo_year])
